...
dDocent uses FreeBayes to call SNPs and write a VCF file `TotalRawSNPs.vcf`.
## This dataset is filtered to remove low quality and artefactual SNP sites, and potential paralogs.
## This dataset is also filtered to remove low quality individuals based on missing data, minimum/maximum read depth, and genotype call rates.
## Because this is two separate ddRAD libraries combined, loci that are inconsistent across libraries (i.e. library effects) are also removed.

chart.sh on raw data
# charts.sh script is run multiple times throughout the SNP filtering to closely assess filtering
```{bash}
...
#The dataset is now filtered to retain only the individuals putatively identified using cyt b
#Once the vcf is split by putatively identified species, each separate dataset will be filtered for missing data
  ## This is done to remove loci that are only found in one species or the other. (i.e. 90% missing in N. megalops would indicate it was a locus most likely found only in N. amabilis, vice versa).
  ## Want to retain loci that are similar between the two species.
...
```
## Filter vcf to retain only individuals identified as Notropis megalops using cyt b 
```{bash}
...
#Missing data >90% 
## 90% missing data would indicate loci only present in N.amabilis (i.e. 90-100% missing in N.megalops)
```{bash}
...
length(AmaInds) #25 N.amabilis individuals
```
## Filter vcf to retain only individuals identified as Notropis amabilis using cyt b 
```{bash}
...
## Remove all snps that are only found in either species
```{bash}
...
#remove the duplicate snps from the doubleton vcf
...
#The Final_Filter.count in the SOL folder needs to be exported and evaluated using SOL_Filter.xlsx to determine the best filtering path

#A.1.1.1.2.2.3.1.2.SNP.finalb.1 retains the most SNPs, contigs, and individuals

cp vcf/A.1.1.1.2.2.3.1.2.SNP.finalb.1.recode.vcf ../
cd ..
charts.sh A.1.1.1.2.2.3.1.2.SNP.finalb.1.recode.vcf
```
         *** dataset currently contains 28,723 SNPs, 4,377 contigs, 295 individuals***

9. Removing loci with high depth
```{bash}
...
```{r}
#Check how many loci > 0.5 heterozygosity
...
dplyr::count(SNPs) 
```
   *** dataset currently contains 26,139 SNPs, 4,310 contigs, 293 individuals***
...
## Use `notr_haplo_inds` to create `popmap` as a tab-separated file of individuals and their population designation with one individual per line
```{bash}
...
#3. > 90% Missing data
## Select the individuals included in cyt b dataset and then separate by putative identified species
  ## Doing it again post haplotyping to look for loci
...
## 90% missing data would indicate that it is a loci only present in the N.megalops 
```{bash}
#output missing data file by site 
...
##Remove loci from genind - genind output by rad haplotyper
```{r}
#Import genepop file
...
```
              ***Final SNP filtered dataset contains***
...
#Read in popmap and assign strata
```{r}
...
## Identify any loci with high genotyping error.
```{r}
...
##Remove one sample from each pair of duplicates
```{r}
#From genind
...
#Identify Outliers by Library using OutFLANK
...
# Create a vector listing locus name
...
## Bayescan needs at least 3 groups to run, so because there are only two libraries, the data gets separated by Index within Library so there are 8 total groups, 4 indicies per library.

## Prepare files for Bayescan
```{r}
#Read in popdata
popdata <- read.csv(file="/home/kdye/Nmegalops/data/pop.data.csv", header = TRUE)
...
##Write the Genepop to libeffects folder
...
```{bash}
#Need input file and genepop_bayescan.spid in directory
...
dplyr::count(fst, qval <= 0.05)  
dplyr::count(fst, qval <= 0.01) 
dplyr::count(fst, qval <= 0.001) 
```
...
PCA2 <- ggplot(PC_Inds, aes(x = Axis1, (y = Axis2*-1), fill = SPECIES)) +
  geom_point(alpha = 0.75, shape =  21, size = 2.5) +
